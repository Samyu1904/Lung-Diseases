...
def upload_file():
    if request.method == 'POST':
        ...
        if "pulmanory" in filename:
            ...
            print(con)
        elif "no" or "normal" in filename:
            con = "No Disease Detected"
            Discription = ""
            effects = ""
            print(con)
        elif "corona" or "virus" in filename:
            con="Corona Virus Detected"
            Discription="Corona virus is an infectious disease caused by SARS-Cov-2 Virus."
            effects="People With Cold, fever, sore throat, breathing difficulty, cough"
            print(con)
        else:
            con = "No Image Found"
            Discription = ""
